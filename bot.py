import os from datetime import datetime from functools import wraps from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup, InputMediaPhoto from telegram.ext import ApplicationBuilder, CommandHandler, ContextTypes, CallbackQueryHandler, MessageHandler, filters
...
ADMINS = [123456789, 7432801922, 1776669221]  # Updated with your Telegram user ID
...
def restricted_to_admins(func): @wraps(func) async def wrapped(update: Update, context: ContextTypes.DEFAULT_TYPE, *args, **kwargs): user_id = update.effective_user.id if user_id not in ADMINS: return await update.message.reply_text("🚫 This command is only for admins.") return await func(update, context, *args, **kwargs) return wrapped

Generate a unique order ID

def generate_order_id(): return f"LYC{datetime.now().strftime('%y%m%d%H%M%S')}"

Start Command

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE): keyboard = [ [InlineKeyboardButton("📸 Instagram Services", callback_data='instagram')], [InlineKeyboardButton("▶️ YouTube Services", callback_data='youtube')], [InlineKeyboardButton("📲 Telegram Services", callback_data='telegram')], [InlineKeyboardButton("📖 FAQs", callback_data='faq')], [InlineKeyboardButton("📦 Track My Order", callback_data='track')], ] reply_markup = InlineKeyboardMarkup(keyboard) await update.message.reply_text("Welcome to LycanSmm! Select a service:", reply_markup=reply_markup)

Admin Panel Command

@restricted_to_admins async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE): keyboard = [ [InlineKeyboardButton("📥 View Recent Orders", callback_data='admin_view_orders')], [InlineKeyboardButton("✅ Confirm Order", switch_inline_query_current_chat="Confirm Order: LYC")], [InlineKeyboardButton("❌ Reject Order", switch_inline_query_current_chat="Reject Order: LYC")], [InlineKeyboardButton("💰 Check Payments", callback_data='admin_check_payments')], ] await update.message.reply_text("🛠️ Admin Control Panel", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE): user_message = update.message.text if user_message.upper().startswith("LYC"): await update.message.reply_text( f"🔎 Tracking Order ID: {user_message}\nStatus: In queue / Processing.\nYou'll get a confirmation shortly." ) elif user_message.lower().startswith("payment screenshot") or update.message.photo: await update.message.reply_text("📩 Screenshot received. Our team will verify and confirm soon.") else: await update.message.reply_text( f"✅ Link received: {user_message}\nOur team will process shortly." )

Main

if name == 'main': app = ApplicationBuilder().token(TOKEN).build() app.add_handler(CommandHandler("start", start)) app.add_handler(CommandHandler("admin", admin_panel)) app.add_handler(CallbackQueryHandler(button, pattern=".*")) app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_message)) app.add_handler(MessageHandler(filters.PHOTO, handle_message)) print("Bot is running...") app.run_polling()
